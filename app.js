...
function renderCategories(query=''){
  ...
  filtered.forEach(c=>{
    ...
    btn.innerHTML=`<div class="cat-banner"><img src="${c.image}" alt="" loading="lazy" onerror="if(!this.dataset.f){this.dataset.f=1;this.src='assets/heroes/'+'${c.image}'}else{this.parentElement.innerHTML='<div class=cat-fallback>${c.icon}</div>'}"></div><div class="cat-label"><span>${c.icon} ${nameOf(c)}</span><em>${c.items.length} ${t('products')}</em></div>`;
...
function renderCategory(id){
  ...
  $('#categoryHero').style.backgroundImage=`url('${c.image}'), url('assets/heroes/${c.image}')`;$('#heroMeta').textContent=`${c.icon} ${nameOf(c)} · ${c.items.length} ${t('products')}`;
  const grid=$('#productGrid');grid.innerHTML='';c.items.forEach((i,idx)=>{
    const card=document.createElement('article');card.className='product-card';const thumb=i[3]?`<img src="${i[3]}" alt="" loading="lazy" onerror="if(!this.dataset.f){this.dataset.f=1;this.src='assets/products/'+'${i[3]}'}else{this.parentElement.innerHTML='<span>${c.icon}</span>'}">`:`<span>${c.icon}</span>`;card.innerHTML=`<div class="product-thumb">${thumb}</div><div class="product-info"><h3>${state.lang==='ar'?i[1]:i[0]}</h3><p>${state.lang==='ar'?i[0]:i[1]}</p><p class="price">${money(i[2])}</p></div><button class="add-btn" aria-label="Add">+</button>`;
...
function openProductModal(catId,itemIndex){
  ...
  $('#modalThumb').style.backgroundImage=i[3]?`url('${i[3]}'), url('assets/products/${i[3]}')`:`url('${c.image}'), url('assets/heroes/${c.image}')`;$('#modalCategory').textContent=nameOf(c);$('#modalName').textContent=state.lang==='ar'?i[1]:i[0];$('#modalPrice').textContent=money(i[2]);$('#modalQty').textContent='1';$('#itemNote').value='';
